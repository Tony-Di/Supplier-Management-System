...
try {
  ...
  const existing = await countAuditEntries(client);
  if (existing.rows > 0) {
    console.error(
      `Refusing to import: audit_logs already has ${existing.rows} row(s). ` +
        `This script is not safe to run more than once. If you intended to re-import, ` +
        `clear the table yourself first and re-run.`,
    );
    process.exitCode = 1;
  } else {
